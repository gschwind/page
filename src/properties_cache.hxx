...
class properties_cache_t {
	...
	struct _key_t {
		...
		_key_t(Window w, Atom prop) :
				w(w), prop(prop) {
		}

		bool operator<(_key_t const & k) const {
			if (w < k.w) {
				return true;
			} else {
				return prop < k.prop;
		...
		bool operator==(_key_t const & k) const {
			return w == k.w and prop == k.prop;
		}

	};

	typedef pair<Window, Atom> _x_key_t;
	...
	bool _unsafe_get_window_property(Display * dpy, Window win, Atom prop,
		...
		if (::page::get_window_property_void<T>(dpy, win, prop, type, &tmp.nitems, &tmp.data)) {
			...
			_cache[_x_key_t(win, prop)] = _value_t(type, _format<T>::size);
...
public:
	...
	bool get_window_property(Display * dpy, Window win, Atom prop, Atom type,
			vector<T> & v) {
		map<_x_key_t, _value_t>::iterator x = _cache.find(_x_key_t(win, prop));
		if (x == _cache.end()) {
			++_miss;
	...
	void erase(Window win) {
		map<_x_key_t, _value_t>::iterator x = _cache.begin();
		while(x != _cache.end()) {
