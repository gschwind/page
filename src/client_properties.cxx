...
void client_properties_t::select_input(uint32_t mask) {
	_dpy->select_input(_id, mask|XCB_EVENT_MASK_PROPERTY_CHANGE|XCB_EVENT_MASK_STRUCTURE_NOTIFY);
}

void client_properties_t::set_focus_state(bool is_focused) {
	if (is_focused) {
...
void client_properties_t::select_input_shape(bool x) {
	xcb_shape_select_input(cnx()->xcb(), _id, x?1:0);
}

void client_properties_t::grab_button (
...
void client_properties_t::ungrab_button(uint8_t button, uint16_t modifiers) {
	xcb_ungrab_button(_dpy->xcb(), button, _id, modifiers);
}

void client_properties_t::send_event (
                uint8_t           propagate  /**< */,
                uint32_t          event_mask  /**< */,
                const char       *event  /**< */) {
...
void client_properties_t::set_input_focus(int revert_to, xcb_timestamp_t time) {
	xcb_set_input_focus(_dpy->xcb(), revert_to, _id, time);
}

void client_properties_t::move_resize(rect const & size) {
	_dpy->move_resize(_id, size);
}

void client_properties_t::fake_configure(rect const & location, int border_width) {
...
void client_properties_t::delete_window(xcb_timestamp_t t) {
...
void client_properties_t::set_border_width(uint32_t width) {
	xcb_configure_window(_dpy->xcb(), _id, XCB_CONFIG_WINDOW_BORDER_WIDTH, &width);
}

void client_properties_t::unmap() {
	xcb_unmap_window(_dpy->xcb(), _id);
}

void client_properties_t::reparentwindow(xcb_window_t parent, int x, int y) {
	xcb_reparent_window(_dpy->xcb(), _id, parent, x, y);
}

void client_properties_t::xmap() {
	xcb_map_window(_dpy->xcb(), _id);
}

void client_properties_t::delete_net_wm_state() {
	__net_wm_state.clear(_dpy->xcb(), _dpy->_A, _id);
}

void client_properties_t::delete_wm_state() {
	_wm_state.clear(_dpy->xcb(), _dpy->_A, _id);
}

void client_properties_t::add_to_save_set() {
