...
i_rect tiny_theme_t::compute_notebook_bookmark_position(
		i_rect const & allocation) const {
	return i_rect(allocation.x + allocation.w - 4 * 17 - 2, allocation.y + 2,
			16, 16).floor();
}

i_rect tiny_theme_t::compute_notebook_vsplit_position(
		i_rect const & allocation) const {
...
i_rect tiny_theme_t::compute_notebook_hsplit_position(
		i_rect const & allocation) const {
...
i_rect tiny_theme_t::compute_notebook_close_position(
		i_rect const & allocation) const {
...
i_rect tiny_theme_t::compute_notebook_menu_position(
		i_rect const & allocation) const {
...
void tiny_theme_t::rounded_i_rect(cairo_t * cr, double x, double y,
...
static void draw_notebook_border(cairo_t * cr, i_rect const & alloc,
	...
	CHECK_CAIRO(tiny_theme_t::cairo_rounded_tab(cr, tab_area.x + 0.5, tab_area.y + 0.5, tab_area.w - 1.0, tab_area.h - 1.0, 7.0));
...
void tiny_theme_t::render_split(cairo_t * cr,
...
void tiny_theme_t::render_empty(cairo_t * cr, i_rect const & area) const {
	CHECK_CAIRO(cairo_save(cr));
	cairo_clip(cr, area);
	cairo_set_operator(cr, CAIRO_OPERATOR_SOURCE);
	if (background_s != nullptr) {
...
void tiny_theme_t::render_floating(theme_managed_window_t * mw) const {
...
void tiny_theme_t::render_floating_base(
...
void tiny_theme_t::render_popup_notebook0(cairo_t * cr, icon64 * icon, unsigned int width,
...
void tiny_theme_t::render_popup_move_frame(cairo_t * cr, icon64 * icon, unsigned int width,
		unsigned int height, std::string const & title) {
	render_popup_notebook0(cr, icon, width, height, title);
}

void tiny_theme_t::draw_hatched_i_rect(cairo_t * cr, int space, int x, int y, int w, int h) const {
...
void tiny_theme_t::create_background_img() {
...
void tiny_theme_t::render_popup_split(cairo_t * cr, theme_split_t const * s,
...
void tiny_theme_t::render_menuentry(cairo_t * cr, theme_dropdown_menu_entry_t const & item, i_rect const & area, bool selected) {
...
void tiny_theme_t::cairo_rounded_tab(cairo_t * cr, double x, double y, double w, double h, double radius) {
...
void tiny_theme_t::cairo_rounded_tab2(cairo_t * cr, double x, double y, double w, double h, double radius) {
...
void tiny_theme_t::cairo_rounded_tab3(cairo_t * cr, double x, double y, double w, double h, double radius) {
...
color_t const & tiny_theme_t::get_selected_color() const {
	return notebook_selected_background_color;
}

color_t const & tiny_theme_t::get_normal_color() const {
